...
3. Mantener `DataSource:Mode=Disabled` durante la validacion visual y luego configurar reglas aprobadas y cadena ODBC integrada.
4. Cambiar a `DataSource:Mode=Odbc` solo en la ventana coordinada con el DBA.
...
Conservar el artefacto anterior, detener el Application Pool, restaurar el directorio anterior y arrancar el pool. Si existe duda sobre datos o permisos, volver primero a `DataSource:Mode=Disabled`; la aplicacion no escribe en LOLCLI.
