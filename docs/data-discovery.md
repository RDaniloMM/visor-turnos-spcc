...
El 2026-09-19 se comprobó que la conexión ODBC y las lecturas simples responden en menos de un segundo, mientras que la consulta anterior con un `OUTER APPLY` por cada cita podía exceder el timeout. La consulta vigente limita `am_consulta` a los `Queue:RecentConsultationRows` `numcon` más recientes y toma el último por `invnum`. Es una lectura acotada y suficiente para detectar actos recién creados; si la carga real supera esa ventana entre sondeos, el DBA debe aprobar un índice o una vista por `invnum`.
...
El 2026-09-22 se confirmó en la instancia real que `am_diagnosticos` posee `numcon` y `diacod`, y que el código `Z53.8` aparece en actos de la jornada. La consulta productiva no lee la descripción ni expone códigos: deriva únicamente `has_no_show_diagnosis` mediante `EXISTS` por el `numcon` del último acto. La clave de `am_diagnosticos` comienza por `numcon`, por lo que esta comprobación se mantiene acotada a los actos recientes que ya lee el visor.

La misma ventana de actos recientes marca `has_prior_closed_act_without_no_show` cuando el nuevo `numcon` de una cita tiene un acto anterior con `stacon = P` que no contiene `Z53.8`. Es una señal de ambigüedad, no una prueba de atención clínica: bloquea el llamado automático y deja que el paciente se acerque al consultorio. Como la lectura está limitada por `Queue:RecentConsultationRows`, la garantía completa para actos muy antiguos requiere que el DBA apruebe un índice o una vista que permita buscar por `invnum` sin escanear toda `am_consulta`.
