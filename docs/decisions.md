...
| --- | --- | --- | --- |
| 2026-09-16 | Cada llamado anuncia al paciente inicialmente y repite el aviso a los 30 segundos. A los 60 segundos, si no existe prefactura válida, desaparece del llamado, se reubica al final de la cola de su consultorio/médico y puede volver a ser llamado hasta cuatro veces. Cuando LOLCLI genera una prefactura válida, el turno pasa internamente a `EnAtencion`, desaparece de la interfaz pública y el visor llama de inmediato al siguiente de su consultorio/médico; no espera `statte=S`. Después del cuarto intento el visor no escribe ni elimina la cita; esa acción corresponde a LOLCLI. | Responsable del proyecto, solicitud directa | Aplicada |
...
| 2026-09-16 | La cola consulta solo la jornada actual de Cuajone. Las listas públicas muestran citas pendientes desde la hora actual en adelante; una cita anterior solo permanece visible si ya fue activada como llamado. Cada consultorio inicia automáticamente un turno solo durante el mismo minuto de `citdat` y continúa con el siguiente elegible de su propia cola; no inicia llamadas para minutos anteriores. `cithll` no bloquea el llamado porque nunca se llena. Los casos cerrados o con prefactura se excluyen de la interfaz pública. | Responsable del proyecto, solicitud directa | Aplicada |
